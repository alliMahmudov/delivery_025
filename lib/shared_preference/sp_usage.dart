...
// Save total amount in SharedPreferences
  Future<void> saveTotalAmount(double totalAmount) async {
...
// Load total amount from SharedPreferences
  Future<double> loadTotalAmount() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    double totalAmount = prefs.getDouble('totalAmount') ?? 0.0;
    notifyListeners();
    return totalAmount;
  }

// Example of updating total amount in cart and saving it
  void updateCart(List<CartItemSP> cartItems) async {
    double totalAmount = calculateTotalAmount(cartItems);

    // Save total amount in SharedPreferences
    await saveTotalAmount(totalAmount);
  }*/
